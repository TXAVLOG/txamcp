...
if (process.env.ENABLE_HTTP_GATEWAY === 'true') {
    ...
    app.listen(PORT, () => {
        log.success(`Txa MCP Gateway v3.0.1 running on http://localhost:${PORT}`);
    }).on('error', (err) => {
...
async function main() {
    try {
        ...
        log.success(`Txa_MCP Core Engine v3.0.1 Online - Plan: ${auth.user.plan_name}`);
        log.info(`Authenticated as ${auth.user.username}`);
        
        // Register tools BEFORE connecting
        await registerTools();
    } catch (err) {
        log.success("Txa_MCP Core Engine v3.0.1 Online (Offline Mode)");
